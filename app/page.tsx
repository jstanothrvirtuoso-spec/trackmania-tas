        ...
        <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 lg:px-8">
          <header className="mb-10 rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-xl shadow-slate-950/30 backdrop-blur-md">
            ...
            <p className="mt-6 max-w-2xl text-slate-400 sm:text-base">
              Tracks all TAS contributions across every leaderboard. Each author
              earns one TAS per entry they appear on. Total time saved and
              contributions are fractional shares based on the number of authors
              per TAS.
            </p>
          </header>
            ...
            <table className="min-w-full divide-y divide-slate-800 text-center text-sm">
              ...
              <tbody className="divide-y divide-slate-800">
                {sortedAuthorStats.map((author) => (
                  <tr
                    key={author.author}
                    className="border-b border-slate-800 last:border-b-0 hover:bg-slate-900/50 transition"
                  >
                    <td className="px-2 py-2 text-slate-100">{author.author}</td>
                    <td className="px-2 py-2 text-slate-100">{author.tases}</td>
                    <td className="px-2 py-2 text-slate-100">
                      {author.contributions.toFixed(2)}
                    </td>
                    <td className="px-2 py-2 text-slate-300">
                      {formatTime(author.totalSaved, false, false, false)}
                    </td>
                  </tr>
                ))}
              </tbody>
